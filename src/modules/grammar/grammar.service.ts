...
export class GrammarService {
  ...
  private detectPassiveVoice(text: string, issues: GrammarIssueDto[]): void {
    ...
    while ((match = regex.exec(text)) !== null) {
      issues.push({
        ...
        message: 'Passive voice detected — prefer active voice for stronger impact',
  ...
  private generateSummary(
    ...
    if (errorCount > 0) details.push(`${errorCount} error${errorCount > 1 ? 's' : ''}`);
    if (warningCount > 0) details.push(`${warningCount} warning${warningCount > 1 ? 's' : ''}`);
    if (infoCount > 0) details.push(`${infoCount} suggestion${infoCount > 1 ? 's' : ''}`);
    parts.push(`(${details.join(', ')})`);

    if (context) {
      parts.push(`in ${context.replace('_', ' ')} context`);
    }
